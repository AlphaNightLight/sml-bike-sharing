...
def get_historical_weather(city, start_date,  end_date, latitude, longitude):
    ...
    params = {
        ...
        "end_date": end_date,
        "daily": ["weather_code", "apparent_temperature_mean", "daylight_duration", "precipitation_sum", "wind_speed_10m_max"]
    }
    ...
    apparent_temperature_mean = daily.Variables(1).ValuesAsNumpy()
    ...
    daily_data["weather_code"] = weather_code.astype('int32')
    daily_data["apparent_temperature_mean"] = apparent_temperature_mean
...
def get_historical_weather_aq(city, start_date,  end_date, latitude, longitude):
...
def get_hourly_weather_forecast_aq(city, latitude, longitude):
...
def backfill_predictions_for_monitoring_lagged(weather_fg, air_quality_df, monitor_fg, model):
    ...
    features_df = pd.merge(features_df, air_quality_df[['date','pm25_lagged_1', 'pm25_lagged_2', 'pm25_lagged_3']], on="date")

    features_df['predicted_pm25'] = model.predict(features_df[['pm25_lagged_1', 'pm25_lagged_2', 'pm25_lagged_3', 'temperature_2m_mean', 'precipitation_sum', 'wind_speed_10m_max', 'wind_direction_10m_dominant']])
    df = pd.merge(features_df, air_quality_df[['date','pm25','street','country']], on="date")
    df['days_before_forecast_day'] = 1
    hindcast_df = df
    df = df.drop('pm25', axis=1)
...
def backfill_predictions_for_monitoring_berlin(weather_fg, air_quality_df, monitor_fg, model):
    ...
    features_df = pd.merge(features_df, air_quality_df[['date', 'street', 'pm25_lagged_1', 'pm25_lagged_2', 'pm25_lagged_3']], on="date")
    features_df['street'] = features_df['street'].astype("category")

    features_df['predicted_pm25'] = model.predict(features_df[['street', 'pm25_lagged_1', 'pm25_lagged_2', 'pm25_lagged_3', 'temperature_2m_mean', 'precipitation_sum', 'wind_speed_10m_max', 'wind_direction_10m_dominant']])
    df = pd.merge(features_df, air_quality_df[['date','pm25','street','country']], on=["date","street"])
    df['days_before_forecast_day'] = 1
    hindcast_df = df
    df = df.drop('pm25', axis=1)
    monitor_fg.insert(df, write_options={"wait_for_job": True})
